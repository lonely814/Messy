...
## 更新日志
### v1.14.0（换算比例改「1 积分 = X 元」口径，2026-09-22）

- **显示口径改「1 积分 = X 元」**：比「几分 = 1 元」直观（填单价而不是填倒数），label 直接写成 `1 积分 = [0.0667] 元`
- **存储口径不变**：localStorage 仍是 `_lt_rate` = 积分/元，面板显示 `1/_ltRate`（4 位小数），输入 `y` 元/积分时存 `1/y` —— `_ltRmb` 的除法逻辑零改动
- 已存旧值兼容：显示 `1/旧值`，新输入存 `1/新输入`，两边共用一个键
