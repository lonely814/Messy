...
'#libtv-help {',
'  /* 定位(left/top)由 main.js _showHelp 动态接管，跟随 FPS 面板位置；这里不再声明 right/bottom */',
...
'/* ── 提示词悬浮按钮：极简描边一族 ──',
'   5 个槽位共用一套视觉语言（右键循环切换）：',
'   不位移、不缩放、不发光，hover 只变颜色；1px 描边 + currentColor 线性图标。 */',
'#libtv-pbtn {',
...
'  -webkit-backdrop-filter: none !important;',
'  backdrop-filter: none !important;',
'  transition: background .18s var(--ease-out), border-color .18s var(--ease-out), color .18s var(--ease-out), width .22s var(--ease-out) !important;',
'}',
'#libtv-pbtn:hover {',
'  background: rgba(var(--accent-light-rgb),0.12) !important;',
...
'/* 槽位 0：描边圆（基准，无额外覆盖） */',
'',
'/* 槽位 1：幽灵 —— 连描边都省，只剩图标；hover 才浮出淡底 */',
'#libtv-pbtn.s1 { border-color: transparent !important; color: rgba(255,255,255,0.4) !important; }',
'#libtv-pbtn.s1:hover { border-color: transparent !important; color: #fff !important; }',
'',
'/* 槽位 2：小号 28px，hover 长回 36px */',
'#libtv-pbtn.s2 { width: 28px !important; height: 28px !important; }',
'#libtv-pbtn.s2:hover { width: 36px !important; height: 36px !important; }',
'',
'/* 槽位 3：展开 —— hover 从圆长成胶囊，滑出「提示词」 */',
'#libtv-pbtn.s3 { width: 36px !important; height: 36px !important; border-radius: 18px !important; white-space: nowrap !important; }',
'#libtv-pbtn.s3:hover { width: 112px !important; }',
'#libtv-pbtn.s3 .s3-label { opacity: 0; transition: opacity .18s var(--ease-out) .06s; }',
'#libtv-pbtn.s3:hover .s3-label { opacity: 1; }',
'',
'/* 槽位 4：纯文字「P」 */',
'#libtv-pbtn.s4 { background: transparent !important; }',
'',
...
'.lt-settings-toggle span:first-child { font-size:13.5px; color:rgba(255,255,255,0.62); white-space:nowrap; }',
...
'.lt-model-sel { display:inline-flex;align-items:center;gap:5px;max-width:170px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);color:#aaa;border-radius:8px;padding:6px 10px;font:12px/1.5 -apple-system,BlinkMacSystemFont,sans-serif;outline:none;cursor:pointer;transition:border-color .15s var(--ease-out),color .15s var(--ease-out); }',
'.lt-model-sel:hover { border-color:rgba(var(--accent-light-rgb),0.4);color:#fff; }',
'.lt-model-sel:focus { border-color:var(--accent-light); }',
'.lt-model-sel:disabled { opacity:0.5;cursor:not-allowed; }',
'/* 自绘模型下拉弹层（对齐 .lt-settings-mitem 观感） */',
'.lt-model-item { display:flex;align-items:center;gap:6px;padding:7px 10px;border-radius:6px;font-size:11.5px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;color:rgba(255,255,255,0.65);cursor:pointer;transition:background .12s var(--ease-out),color .12s var(--ease-out);white-space:nowrap;overflow:hidden;text-overflow:ellipsis; }',
'.lt-model-item:hover { background:rgba(var(--accent-light-rgb),0.12);color:#fff; }',
'.lt-model-item.cur { color:var(--accent-light);background:rgba(var(--accent-light-rgb),0.08); }',
...
'',
'/* ===== 积分⇄人民币换算标签（inject.js 的 _ltCreditScan 把文本打在 data-lt-rmb 上） =====',
'   走伪元素而不是插节点：React 重渲染碰不到伪元素，也不会污染站点元素的 textContent */',
'[data-lt-rmb]::after {',
'  content: attr(data-lt-rmb);',
'  flex-shrink: 0;',
'  font-size: 10px;',
'  line-height: 1;',
'  font-weight: 400;',
...
'      /* 诊断：当前浮动图标状态 */',
'      _ltScanStats.icons=[];',
'      _ltScanStats.vp=window.innerWidth+"x"+window.innerHeight+" v"+"1.14.0";',
...
'        +\'<div style="padding:4px 12px 6px;display:flex;gap:6px;flex-shrink:0;"><button style="\'+_btnP+\'" id="lt-ap-go">\\u2728 \\u6267\\u884c</button><span id="lt-ap-model-sel" style="display:inline-flex;align-items:center;"></span><button style="\'+_btnG+\'" id="lt-ap-fill">\\u4ece\\u8f93\\u5165\\u6846\\u83b7\\u53d6</button><button style="\'+_btnG+\'" id="lt-ap-clr">\\u6e05\\u7a7a</button></div>\'',
...
'        +"<span style=\\"font-size:10px;color:rgba(255,255,255,0.28);\\">"+(fromCache?"\\u2713 \\u7f13\\u5b58\\uff0c\\u70b9\\u5217\\u8868\\u9009\\u62e9":"\\u6a21\\u578b\\u5217\\u8868")+"</span>"',
'        +"<span id=\\"lt-set-models-refresh\\" style=\\"font-size:10px;color:var(--accent-light);cursor:pointer;padding:1px 6px;border-radius:4px;\\">\\ud83d\\udd04 \\u91cd\\u65b0\\u62c9\\u53d6</span></div>";',
...
'      row.innerHTML="<input class=\\"lt-settings-inp\\" id=\\"lt-set-preset-name\\" placeholder=\\"\\u9884\\u8bbe\\u540d\\u79f0\\uff0c\\u5982 \\"\\u4e2d\\u8f6c\\u7f51\\u5173\\"\\" style=\\"flex:1;\\">"',
'        +"<button class=\\"lt-settings-btn lt-settings-btn-primary lt-settings-btn-sm\\" id=\\"lt-set-preset-ok\\">\\u4fdd\\u5b58</button>"',
'        +"<button class=\\"lt-settings-btn lt-settings-btn-ghost lt-settings-btn-sm\\" id=\\"lt-set-preset-cancel\\">\\u53d6\\u6d88</button>";',
'      this.parentNode.insertBefore(row,this.nextSibling);',
'      var inp=row.querySelector("#lt-set-preset-name");',
'      inp.value="\\u9884\\u8bbe "+(apiPresets.length+1);',
...
'        _ltApiStatus("\\u2713 \\u5df2\\u52a0\\u8f7d\\u7f13\\u5b58\\uff08"+cached.models.length+" \\u4e2a\\u6a21\\u578b\\uff0c\\u53ef\\u70b9\\u5217\\u8868\\u5185\\u91cd\\u65b0\\u62c9\\u53d6\\uff09",true);',
'        return;',
'      }',
