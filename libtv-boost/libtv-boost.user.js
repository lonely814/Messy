...
(function(){
    ...
    style.textContent = [
        ...
        '#libtv-prompt .ltp-item::before { content:""; position:absolute; inset:0; border-radius:8px; opacity:0; transition:opacity .25s; background:radial-gradient(300px circle at var(--mx,50%) var(--my,50%),rgba(var(--accent-light-rgb),0.18),transparent 55%); pointer-events:none; }',
...
'          +\'</div>\'',
'          +\'<div class="ltp-ai-custom" id="ltp-ai-custom"><textarea id="ltp-ai-sys" class="ltp-ai-sys" placeholder="在此输入自定义 system prompt..." rows="2"></textarea></div>\'',
...
'          if(_ltAICurTask==="custom"){_ltCustomEl.style.display="block";document.getElementById("ltp-ai-sys").value=_ltAICustomSys();document.getElementById("ltp-ai-sys").focus();}',
...
'          var sysPrompt=_ltAICurTask==="custom"?_ltAICustomSys():(_ltAIPresets[_ltAICurTask]?_ltAIPresets[_ltAICurTask].sys:"");',
'          if(_ltAICurTask==="custom"&&!sysPrompt.trim()){document.getElementById("ltp-ai-status").textContent="❌ 自定义模式请先填写 system prompt";return;}',
...
'    function _ltGetCookies(){return document.cookie;}',
    ...
    '    function _ltAccList(){try{return JSON.parse(localStorage.getItem("_lt_accounts")||"[]");}catch(e){return [];}}',
    '    function _ltAccSave(name){var a=_ltAccList();a.push({id:Date.now().toString(36),name:name,cookies:_ltGetCookies(),ls:_ltSaveLS(),created:Date.now()});try{localStorage.setItem("_lt_accounts",JSON.stringify(a));}catch(e){}}',
    '    function _ltAccRefresh(id){var a=_ltAccList();for(var i=0;i<a.length;i++){if(a[i].id===id){a[i].cookies=_ltGetCookies();a[i].ls=_ltSaveLS();a[i].created=Date.now();try{localStorage.setItem("_lt_accounts",JSON.stringify(a));}catch(e){}break;}}_ltAccPanel();}',
    ...
    '    function _ltAccDelete(id){var a=_ltAccList();for(var i=0;i<a.length;i++){if(a[i].id===id){a.splice(i,1);try{localStorage.setItem("_lt_accounts",JSON.stringify(a));}catch(e){}break;}}_ltAccPanel();}',
...
'    document.getElementById("ltp-close").onclick=function(){div.remove();};',
    ...
    '      +"<div class=\\"lt-settings-about\\">LibTV Canvas Boost v1.9.9<br>\\u6e90\\u7801\\u6a21\\u5757\\u5316\\u6784\\u5efa\\uff0c\\u63d0\\u4f9b\\u6027\\u80fd\\u4f18\\u5316\\u3001\\u89c6\\u89c9\\u589e\\u5f3a\\u3001AI \\u63d0\\u793a\\u8bcd\\u3001\\u6807\\u7b7e\\u7cfb\\u7edf</div>"',
