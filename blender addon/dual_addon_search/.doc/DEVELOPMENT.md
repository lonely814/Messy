...
| 版本 | 日期 | 变更 |
|------|------|------|
| 3.1.0 | — | 启动耗时统计板块、标签即时生效脏标记机制、删除批量勾选/星标/PRO配置等派生功能、优化权限声明文件；**修复**：bl_pkg 动态分发器导致的面板底部重复出现第二组搜索框（自愈机制 + load_post 提前自愈 + 注销时剔除分发器残留），绘制异常且已写入内容时不再回退追加原生面板只显示错误提示；**其他**：作者署名统一为 loNely |
