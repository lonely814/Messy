...
class TOOL_GGT_gizmo_camera(WorkSpaceTool):
    ...
    def draw_settings(context, layout, tool):
        ...
        icon = 'SCENE_DATA' if props.show_with_scene_camera else 'OBJECT_DATA'
        row.prop(props, 'show_with_scene_camera', text='', icon=icon, toggle=True)
        if props.show_with_scene_camera:
            row.label(text="Auto: Any Camera")
        else:
        ...
        if bpy.context.active_object != None:
            ...
            if props.target_visible == True:
                row.operator("object.target_object",text='',icon='EYEDROPPER')
                row.operator("object.del_target",text='',icon='X')

            row = layout.row(align=True)
            row.prop(props,'show_gizmo',text="Gizmo",icon="PIVOT_CURSOR")
            row.operator("view3d.snap_cursor_to_center",text="",icon="EMPTY_ARROWS")
            row.operator("cursor.resetrotation",text="",icon="ORIENTATION_GIMBAL") 
